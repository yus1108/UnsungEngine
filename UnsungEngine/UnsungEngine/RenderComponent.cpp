...
void RenderComponent::DrawObj(ID3D11DeviceContext * m_pDeviceContext, D3D11_VIEWPORT m_viewport, Renderer * render) {
	if (loadingDone) {
		...
		originalView.r[3] = DirectX::XMVectorSet(0, 0, -50, 1);
...
void RenderComponent::ReadBin(const char* filename, ID3D11Device * m_pDevice, 
	ID3D11DeviceContext * m_pDeviceContext, DirectX::XMFLOAT4 color) {
